...
def train_epoch_minibatch():
    ...
    mini_batches = [epoch[k:k + 4] for k in xrange(0, 8, 4 )]

    for mini_batch in mini_batches:
        epoch_res = minibatch_iteration(mini_batch)
        meanSquaredError += epoch_res['mse']

    epoch_res['mse'] = meanSquaredError/8
    return epoch_res


def minibatch_iteration(mini_batch, **kwargs):
    # print(mini_batch)
    corrections = {}
    for n in NEURON_NAMES:
        corrections[n] = {n2: 0 for n2 in NEURON_NAMES}
        corrections[n]['threshold'] = 0
    ...
    for sample in mini_batch:
        ...
        output = sample[3]
        # run forwards calculation to get output for each neuron
        forward_calc(**{INPUT1: input_output[0], INPUT2: input_output[1], INPUT3: input_output[2], OUTPUT: input_output[3]})
        error = (output - neuron_map.get(OUTPUT).yValue)
        res = {}
        res['Input1'] = input_output[0]
        res['Input2'] = input_output[1]
        ...
        res['Output'] = -1 * (error - input_output[3])
        res['Error'] = error
        epoch_label = "epoch " + str(input_output[0]) + str(input_output[1]) + str(input_output[2]) + str(
            input_output[3])
        mbatch_res[epoch_label] = res
         # compute delta values for each neuron
        compute_gradient(output)

        corrections = minibatch_corrections(corrections)
        # for neuron_from, values_dict in result:
        #     for neuron_to, weights_thresh in values_dict:
        #         corrections[neuron_from][neuron_to] += result[neuron_from][neuron_to]
        mean_squared_error += error**2

    for layer in reversed(layers[1:]):
        for to_node in layer:
            threshold_change = corrections[to_node.name]['threshold']
            to_node.threshold += (ALPHA/len(mini_batch))*threshold_change
            for node_behind in to_node.ahead_of_nodes:
                weight_change = (ALPHA/len(mini_batch))*corrections[node_behind.name][to_node.name]
                node_behind.set_weight_to_node(to_node, node_behind.get_weight_to_node(to_node) + weight_change)

    mbatch_res['mse'] = mean_squared_error
    return mbatch_res


def minibatch_corrections(corrections):
    for layer in reversed(layers[1:]):
        for to_node in layer:
            for node_behind in to_node.ahead_of_nodes:
                weight_change = node_behind.yValue * to_node.delta
                threshold_change = -1 * to_node.delta
                corrections[node_behind.name][to_node.name] += weight_change
                corrections[to_node.name]['threshold'] += threshold_change
    return corrections
